...
func read(r io.Reader, lenbuf []byte) (response *tinyBuf, err error) {
	var length uint64
	...
	ptr := slicePool.Get().(*tinyBuf)
	response = ptr
	if cap(response.b) < int(length) {
		response.b = make([]byte, length)
	} else {
		response.b = response.b[:length]
	}
	_, err = io.ReadFull(r, response.b)
